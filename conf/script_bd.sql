...
-- EXEMPLO DE CONSULTA DE UMA VENDA COM OS ITENS DA VENDA E CLIENTE
SELECT v_idVenda, v_valor_total_venda, v_desconto, l_titulo, iv_quantidade,
iv_valor_total_item, c_nome
FROM Venda, Item_venda, Livro, Cliente
WHERE iv_v_idVenda = v_idVenda
AND l_idLivro = iv_l_idLivro
AND c_idCliente = v_c_idCliente;

ALTER TABLE `recuperacaoav01`.`item_venda` 
ADD COLUMN `iv_data_venda` VARCHAR(45) NULL AFTER `iv_valor_total_item`;
